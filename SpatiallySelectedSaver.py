...
class SpatiallySelectedSaver:
    ...
    def __init__(self, xlsx_filename, analysis_folder, roi_letf_value, roi_right_value, roi_top_value, roi_bottom_value, pix_size_xy):
        ...
        spots_features        =  np.load(analysis_folder + '/spots_features.npy')
        spots_idxs            =  np.load(analysis_folder + '/spots_idxs.npy')
        spots_features_slctd  =  np.zeros_like(spots_features)
        spots_idxs_slctd      =  []

        cnt  =  0
        for uu in range(spots_features.shape[2]):
            ref_x  =  spots_features[:, 5, uu]
            ref_x  =  ref_x[ref_x != 0]
            ref_y  =  spots_features[:, 6, uu]
            ref_y  =  ref_y[ref_y != 0]
            if roi_letf_value < ref_x.mean() <= roi_right_value and roi_top_value < ref_y.mean() <= roi_bottom_value:
                spots_features_slctd[:, :, cnt]  =  spots_features[:, :, uu]
                spots_idxs_slctd.append(spots_idxs[uu])
                cnt  +=  1

        spots_features_slctd  =  spots_features_slctd[:, :, :cnt]
